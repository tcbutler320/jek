...
function foo(){
    ...
    changeDirButNoMove(0, MOVEMENT_SPEED, FACING_DOWN);
    frameCount++;
...
function gameLoop() {
  ...
  if (keyPresses.ArrowUp) {
    ...
    changeDirButNoMove(0, -MOVEMENT_SPEED, FACING_UP);
    hasMoved = true;
  ...
  if (keyPresses.ArrowLeft || keyPresses.Backspace) {
    ...
    changeDirButNoMove(-MOVEMENT_SPEED, 0, FACING_LEFT);
    hasMoved = true;
    ...
    changeDirButNoMove(MOVEMENT_SPEED, 0, FACING_RIGHT);
    console.log('here we go: ' + scrolled);
    hasMoved = true;
  }
 
  /* this function makes the character loop through animatin sequenc, i.e move arms legs */
  if (hasMoved) {
    frameCount++;
    if (frameCount >= FRAME_LIMIT) {
      frameCount = 0;
...
function changeDirButNoMove(deltaX, deltaY, direction) {
  if (
    positionX + deltaX > 0 &&
    positionX + SCALED_WIDTH + deltaX < canvas.width
  ) {
